...
- **本地存储**：对话数据保存在 `localStorage`；绘画历史图片体积较大，保存在 IndexedDB。
...
识别到 URL 配置后，OwnChat 会先弹出确认预览，并对 API Key 做脱敏显示。确认导入后才会写入浏览器本地存储，并自动从地址栏移除 `config` / `config_b64` 参数。
...
- `localStorage`：对话配置、对话会话、绘画配置、主题、当前模式等轻量数据
- IndexedDB：绘画历史和图片数据
